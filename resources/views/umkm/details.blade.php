<div id="modalOverlay" class="fixed inset-0 z-[9999] hidden bg-nunyai-text/80 backdrop-blur-md flex items-center justify-center p-2 md:p-4 transition-opacity duration-300">
    <div class="bg-white w-full max-w-5xl max-h-[95vh] rounded-[2.5rem] md:rounded-[3.5rem] shadow-2xl overflow-y-auto md:overflow-hidden relative flex flex-col md:flex-row animate-modal-up border border-white/20">
        ...
        <div class="w-full md:w-5/12 bg-gray-50 p-6 md:p-10 flex flex-col justify-center items-center space-y-4 border-r border-gray-100 text-left">
            <div class="flex items-center gap-2 self-start mb-2 text-left">
            ...
            <div class="relative w-full group">
                <div class="relative overflow-hidden rounded-[2rem] shadow-lg border-4 border-white aspect-square bg-white">
                    <div id="imageSlider" class="flex transition-transform duration-500 ease-in-out h-full"></div>
                    <button onclick="changeSlide(-1)" class="absolute left-3 top-1/2 -translate-y-1/2 bg-white/80 hover:bg-nunyai-brown hover:text-white p-2 rounded-full shadow-md transition-all opacity-0 group-hover:opacity-100 hidden md:flex">❮</button>
                    <button onclick="changeSlide(1)" class="absolute right-3 top-1/2 -translate-y-1/2 bg-white/80 hover:bg-nunyai-brown hover:text-white p-2 rounded-full shadow-md transition-all opacity-0 group-hover:opacity-100 hidden md:flex">❯</button>
                </div>
        ...
        <div class="w-full md:w-7/12 p-8 md:p-14 bg-white flex flex-col">
            <div class="flex items-center gap-5 mb-8 border-b border-nunyai-bg/20 pb-8 text-left">
            ...
            <div class="space-y-8 flex-grow">
                <div class="text-left">
                    ...
                    <p id="detDescription" class="text-nunyai-text/80 leading-relaxed text-sm md:text-base font-medium text-left"></p>
                </div>

                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 bg-nunyai-bg/10 p-5 md:p-8 rounded-[2.5rem]">
                    <div class="flex items-start gap-3 text-sm text-left group">
                    ...
                    <div class="flex items-center gap-3 text-sm text-left group">
                        <span class="bg-white p-2.5 rounded-xl shadow-sm">⏰</span>
                        <div class="flex flex-col">
                            <span class="text-[9px] font-black text-gray-400 uppercase tracking-widest leading-none mb-1">Jam Operasional</span>
                    ...
                    <div class="flex flex-col gap-2 md:col-span-2">
                        <span class="text-[9px] font-black text-gray-400 uppercase tracking-widest leading-none mb-2 ml-1 text-left">Link Platform & Sosmed</span>
                        <div id="detLinksContainer" class="flex flex-wrap gap-2 justify-start"></div>
                    </div>
                </div>
            </div>

            <div class="mt-8 pt-6 border-t border-gray-50 text-left">
                <a id="modalWaBtn" href="#" target="_blank" class="w-full bg-nunyai-green text-white py-4 rounded-2xl font-black text-[11px] md:text-xs uppercase tracking-[0.2em] shadow-xl hover:bg-nunyai-brown transition-all duration-300 flex justify-center items-center gap-3 active:scale-95 text-center">Hubungi via WhatsApp</a>
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    function showUmkmDetail(id) {
        ...
        // Setup Slider & Links... (Sama seperti sebelumnya)
        setupSlider(data);
        setupLinks(data);
    ...
    function setupSlider(data) {
        ...
        images.forEach((img, idx) => {
            sliderContainer.innerHTML += `<img src="${img.src}" class="w-full h-full object-cover flex-shrink-0" data-label="${img.label}">`;
    ...
    function closeUmkmDetail() { document.getElementById('modalOverlay').classList.add('hidden'); document.body.style.overflow = 'auto'; }
</script>